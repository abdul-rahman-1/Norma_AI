...
export default function AddStaff() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.response?.data?.detail || 'Failed to add staff. Ensure you have the required permissions.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto py-12 px-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-[3rem] p-12 shadow-[0_50px_100px_rgba(0,0,0,0.05)] border border-zinc-100 dark:border-gray-700 relative overflow-hidden">
        <div className="absolute top-0 right-0 p-12 opacity-[0.03] dark:opacity-[0.05] pointer-events-none">
          <Users size={300} className="dark:text-white" />
        </div>
        ...
        <div className="relative z-10">
          <header className="mb-16">
            ...
            <h1 className="text-5xl font-black text-black dark:text-white tracking-tighter uppercase italic leading-none">Register Staff</h1>
            <p className="text-zinc-400 dark:text-gray-500 font-bold text-xs uppercase tracking-[0.4em] mt-4 opacity-60">Initializing personnel profile...</p>
          </header>
          ...
          {success ? (
            ...
            >
              ...
              <h2 className="text-3xl font-black text-black dark:text-white uppercase italic tracking-tighter">Staff Provisioned</h2>
              <p className="text-zinc-500 dark:text-gray-400 font-bold uppercase tracking-widest text-[10px]">Access credentials established...</p>
            </motion.div>
          ) : (
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black text-zinc-400 dark:text-gray-500 uppercase tracking-[0.3em] ml-2">Phone Number (Terminal ID)</label>
                <div className="relative group">
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black text-zinc-400 dark:text-gray-500 uppercase tracking-[0.3em] ml-2">Access Key (Password)</label>
                <div className="relative group">
                  <Lock size={20} className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-300 dark:text-gray-600 group-focus-within:text-[#7c3aed] transition-colors" />
                  <input 
                    type="password" 
                    required
