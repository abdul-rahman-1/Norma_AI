...
export default function AdminDashboard() {
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-12 animate-in fade-in duration-1000 pb-20 relative">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -50 }}
            className="fixed top-10 left-1/2 -translate-x-1/2 z-50 bg-zinc-900 text-white px-8 py-4 rounded-[2rem] flex items-center gap-3 shadow-2xl shadow-zinc-900/20 font-black uppercase tracking-widest text-[10px] border border-white/10"
          >
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10">
        ...
        <div className="flex gap-6">
           <button onClick={() => showToast('Nodes successfully reset and synchronized')} className="nm-button-luxury px-12 py-6 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] text-slate-600 flex items-center gap-4 hover:text-purple-600 active:scale-95 transition-all">
              <RefreshCw size={18} className="text-purple-600" />
              Reset Nodes
           </button>
           <button onClick={() => showToast('System console uplink established')} className="bg-zinc-900 text-white px-12 py-6 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] shadow-2xl shadow-zinc-900/20 hover:bg-purple-600 active:scale-95 transition-all flex items-center gap-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 space-y-12">
           <motion.div variants={itemVariants} className="nm-luxury p-12 rounded-[4rem] border border-white relative overflow-hidden group bg-zinc-900 text-white">
              ...
              <button onClick={() => showToast('Full Security Audit Initiated')} className="w-full mt-20 py-7 rounded-[2rem] bg-white text-zinc-900 text-[10px] font-black uppercase tracking-[0.4em] hover:bg-purple-600 hover:text-white transition-all duration-700 shadow-2xl relative z-10 active:scale-95">
                 Full Security Audit
              </button>
           </motion.div>
